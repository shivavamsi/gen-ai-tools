...
The complete report, expanding the short template in **[library-review.md](library-review.md)**. Fill in every bracketed placeholder. Rate each dimension 1-5 stars using the scale below and back the score with concrete evidence (command output, file paths, line references). Delete any guidance lines before delivering.
...
- **Passed:** [items]
- **Gaps:** [items]
- **Evidence:** [e.g. `uv build` result, `uvx twine check dist/*` output]
...
- **Passed:** [items]
- **Gaps:** [items]
- **Evidence:** [e.g. `uvx bandit -r src/`, `uvx pip-audit` output]
