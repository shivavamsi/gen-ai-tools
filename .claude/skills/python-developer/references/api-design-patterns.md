...
def connect(host, port=5432, tls=True, retry=False, timeout=30):
    ...

connect("db.internal", 6432, True, False, 5.0)   # what is True? what is False?
...
progressive disclosure pattern in **[api-design.md](api-design.md)**).
...
- **Attach context** (offending value, hint) as in the **[api-design.md](api-design.md)** error-handling
  example, rather than baking everything into the message string.

Note that `ConnectionError` and `TimeoutError` above shadow the built-ins of the
same name inside that module, so a later `except TimeoutError` there catches yours
and not the interpreter's. Either accept that deliberately and inherit from the
built-in too, or prefix the names (`LibConnectionError`).
