...
|------|---------|---------|
| ruff | Lint + format | `uv run ruff check src && uv run ruff format --check src` |
...
For full configuration options, see **[code-quality-ruff-config.md](code-quality-ruff-config.md)**.
...
For strict settings and overrides, see **[code-quality-mypy-config.md](code-quality-mypy-config.md)**.
...
For protocols and advanced patterns, see **[code-quality-type-patterns.md](code-quality-type-patterns.md)**.
