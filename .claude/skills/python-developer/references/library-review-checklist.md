...
The detailed expansion of the "Best Practices Checklist" in **[library-review.md](library-review.md)**. Work through each dimension in order. Run the listed command from the repository root and record the result as evidence in your report. Commands assume a `src/` layout with the import package at `src/package/`; adjust the path to the actual package name.
...
- [ ] Built artifacts pass metadata validation — `uvx twine check dist/*`
...
- [ ] No hardcoded secrets, tokens, or credentials — `uvx detect-secrets scan`
- [ ] Static security scan is clean — `uvx bandit -r src/`
- [ ] Dependencies have no known vulnerabilities — `uvx pip-audit`
...
- [ ] Build artifacts are validated before publish — `uvx twine check dist/*`
