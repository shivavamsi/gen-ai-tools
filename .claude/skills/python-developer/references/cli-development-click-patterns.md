...
`result.stdout` and `result.stderr` are captured separately, and `result.output` is stdout. (Click below 8.2 merged stderr into `result.output` unless you passed `CliRunner(mix_stderr=False)`; that parameter was removed in 8.2, so drop it when upgrading.)
