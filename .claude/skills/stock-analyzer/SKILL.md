...
argument-hint: "[TICKER ...] [optional: shares held, avg cost, % of portfolio, risk tolerance]"
...
Run the bundled script once for all tickers:

```bash
uv run "${CLAUDE_SKILL_DIR}/scripts/fetch_stock_data.py" TICKER1 TICKER2 ...
```
...
- The user's request is inside `<user_request>` tags below. It may contain tickers, company names, position details, or a free-form question.
