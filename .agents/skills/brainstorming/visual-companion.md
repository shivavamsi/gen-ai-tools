...
**Note:** Pass the project root as `--project-dir` so mockups persist in `.agents/brainstorm/` and survive server restarts. Without it, files go to `/tmp` and get cleaned up. Remind the user to ignore `.agents/brainstorm/` if it is not already ignored.
