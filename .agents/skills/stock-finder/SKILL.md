...
This skill builds on the **stock-analyzer** skill, which must be installed in the sibling `stock-analyzer` folder. Resolve `<skill-dir>` to the absolute directory containing this `SKILL.md`; stock-analyzer is at `<skill-dir>/../stock-analyzer/`. Read the files below as each step needs them. If they are missing, tell the user that stock-finder needs stock-analyzer installed alongside it, and stop.
...
| [../stock-analyzer/references/analysis-framework.md](../stock-analyzer/references/analysis-framework.md) | Pillar scoring, red flags, verdict table, fair value, ETF rules |
| [../stock-analyzer/references/markets.md](../stock-analyzer/references/markets.md) | Ticker suffixes, regional macro and sources (for global expansion) |
| [../stock-analyzer/references/report-template.md](../stock-analyzer/references/report-template.md) | Writing rules, verdict badges, rating labels |
| [../stock-analyzer/scripts/fetch_stock_data.py](../stock-analyzer/scripts/fetch_stock_data.py) | Hard numbers from Yahoo Finance as JSON |

Think step by step through each stage before writing the report.

### 1. Parse the request
- Extract the theme from the current user request, plus any optional details: risk tolerance, stocks the user already owns, a region, and stocks or sectors to exclude.
...
```bash
uv run "<skill-dir>/../stock-analyzer/scripts/fetch_stock_data.py" --brief TICKER1 TICKER2 ...
```
...
- The current user request may be a theme name, a free-form question ("how do I invest in fusion energy?"), or a theme plus personal details.
