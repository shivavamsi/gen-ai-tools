...
# Objective
Analyze the current repository context to generate a comprehensive `AGENTS.md` file at the repository root. This file is the source of truth for Codex and other AI agents working on the codebase. It must be concise, actionable, and focused on architectural decisions, coding standards, and operational workflows.
...
Generate a Markdown file to be saved at `AGENTS.md`. Follow these repository-instruction best practices:
- Be concise and direct — Codex reads this during work, so avoid prose and prefer bullet points
