...
Downloads an individual YouTube video with customizable quality and format options.
"""
...
DEFAULT_OUTPUT_PATH = str(Path.cwd() / "downloads")


def check_yt_dlp():
    """Check that yt-dlp is installed without modifying the environment."""
    try:
        subprocess.run(["yt-dlp", "--version"], capture_output=True, check=True)
    except (subprocess.CalledProcessError, FileNotFoundError):
        raise RuntimeError(
            "yt-dlp is not installed. Install it with your preferred package manager, then try again."
        )
...
def download_video(url, output_path=DEFAULT_OUTPUT_PATH, quality="best", format_type="mp4", audio_only=False):
    """
