...
| Auth           | dependency guards + **bcrypt**; token scheme per app (see [auth](web-app-architecture-auth.md)) |
...
If you have no reason to deviate, use every row. The rest of this file is the
"how", and the companion references hold copy-pasteable templates.
...
is reachable. Full template in **[web-app-architecture-stack.md](web-app-architecture-stack.md)**.
...
deploy's pre-deploy step. See **[web-app-architecture-stack.md](web-app-architecture-stack.md)**.
...
**[web-app-architecture-payments.md](web-app-architecture-payments.md)**.
...
- **Frontend** — server-rendered Jinja + Tailwind/Alpine (zero JS build, ship fast)
  *or* a decoupled React/Vite SPA on its own origin (richer UI, more moving parts).
  → **[web-app-architecture-frontend.md](web-app-architecture-frontend.md)**
- **Auth scheme** — cookie-JWT (browser app), bearer-JWT/API-key (programmatic),
  ...
  constant; only the token differs. → **[web-app-architecture-auth.md](web-app-architecture-auth.md)**
- **Background work** — inline in the request (simplest), cron-as-worker (the
  ...
  Reach for Celery only when you've outgrown all three. → **[web-app-architecture-background-jobs.md](web-app-architecture-background-jobs.md)**
- **LLM features** — `pydantic-ai` with structured Pydantic outputs, key required
  at boot, model configurable via env. → **[web-app-architecture-frontend.md](web-app-architecture-frontend.md)** has the note.
...
generated in Terraform or marked no-sync and set out of band. → **[web-app-architecture-deployment.md](web-app-architecture-deployment.md)**
...
- **[web-app-architecture-stack.md](web-app-architecture-stack.md)** — pyproject, `main.py`, `db.py`, `config.py`, Makefile templates
- **[web-app-architecture-payments.md](web-app-architecture-payments.md)** — Stripe checkout + idempotent webhook patterns
- **[web-app-architecture-auth.md](web-app-architecture-auth.md)** — the four token schemes and shared dependency guards
- **[web-app-architecture-background-jobs.md](web-app-architecture-background-jobs.md)** — inline vs cron-worker vs Redis queue
- **[web-app-architecture-frontend.md](web-app-architecture-frontend.md)** — Jinja/Tailwind/Alpine vs decoupled SPA (+ LLM note)
- **[web-app-architecture-deployment.md](web-app-architecture-deployment.md)** — Dockerfile, Terraform, migrations, secrets
