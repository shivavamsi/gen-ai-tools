...
def process(
    ...
    tags: Annotated[list[str] | None, typer.Option()] = None,  # repeatable
...
Default a repeatable option to `None`, not `[]` — a mutable default is the
anti-pattern in **[code-quality.md](code-quality.md)** and ruff's `B006` flags it;
normalize with `tags = tags or []` in the body.
...
`result` carries the same `exit_code`, `output`, `stdout`, `stderr`, and `exception` fields as Click's runner — stderr is captured separately (see the note in **[cli-development-click-patterns.md](cli-development-click-patterns.md)** about Click 8.2 removing `mix_stderr`).
