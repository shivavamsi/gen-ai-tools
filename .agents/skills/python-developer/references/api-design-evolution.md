...
release timing, see **[release-management.md](release-management.md)**.
...
- Adding a field to a returned dataclass or `NamedTuple` (see the structured
  return types pattern in **[api-design-patterns.md](api-design-patterns.md)**) — but note callers who unpack a
...
1. **Warn.** Keep the old thing working, but emit a `DeprecationWarning` that
   names the replacement (see the deprecation example in **[api-design.md](api-design.md)**). The old and
...
For the mutable-default-argument trap (`def f(x=[])`), see
**[code-quality.md](code-quality.md)** — it owns that anti-pattern.
